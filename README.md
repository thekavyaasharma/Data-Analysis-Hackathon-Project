# 📊 Data Analysis Hackathon Project 2026
...
│
├── biometric_date_state.ipynb        # Biometric data analysis by date and state
├── biometric_state.ipynb             # Biometric data analysis grouped by state
├── date_age enrollment analysis.ipynb # Enrollment trends by date and age group
├── demo date vs state.ipynb          # Demographic data: date vs state comparison
├── demo state.ipynb                  # Demographic analysis by state
├── enrol bio demo.ipynb              # Combined enrollment, biometric & demographic analysis
├── enrol_state.ipynb                 # Enrollment data grouped by state
└── README.md
```
...
Analyzes biometric capture rates (fingerprints, iris scans, photographs) broken down by Indian state — helping identify which states have the highest/lowest biometric coverage.
...
Compares demographic patterns over time and across states — useful for identifying gaps or surges in specific regions or periods.

### 6. `enrol_state.ipynb`
Enrollment counts aggregated by state — a high-level overview of how many individuals have been enrolled per state.
